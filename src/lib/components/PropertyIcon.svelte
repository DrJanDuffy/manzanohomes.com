...
let iconName = type === 'feature' ? getPropertyFeatureIcon(name) : getPropertyTypeIcon(name);
...
let iconSize = size || defaultConfig.size;
let iconColor = color || defaultConfig.color;

// Generate title if not provided
let iconTitle = title || `${type === 'feature' ? 'Property feature' : 'Property type'}: ${name}`;
</script>
